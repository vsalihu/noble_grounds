...
import type { GalleryImage, GalleryImagePhase } from "@/types/supabase";
...
export function ProjectImageManager({
  ...
  const beforeImages = images.filter((image) => image.phase === "before");
  const afterImages = images.filter((image) => image.phase === "after");
  ...
  async function saveImage(event: FormEvent<HTMLFormElement>) {
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        phase: String(form.get("phase") ?? "after") as GalleryImagePhase,
  ...
  function renderImageCard(image: GalleryImage) {
    return (
      <div
        key={image.id}
        className="overflow-hidden rounded-lg border border-border-soft bg-ivory"
      >
        ...
        {editing?.id === image.id ? (
          <form className="grid gap-3 p-4" onSubmit={saveImage}>
            ...
            <select
              className={inputClass}
              name="phase"
              defaultValue={image.phase}
              aria-label="Image phase"
            >
            ...
            <input
              ...
              aria-label="Image display order"
            />
